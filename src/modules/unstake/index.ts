...
export class UnstakeModule {
  ...
  async fromRoot(params: UnstakeFromRootParams): Promise<UnstakeResult> {
    ...
    return unstakeFromRoot(api, accounts.user, params.hotkey, params.amount);
  }
   ...
   */
  async estimateUnstake(
    hotkey: string,
    amount: string,
    subnet: number = 0
  ): Promise<UnstakeEstimate> {
    const api = await this.apiManager.getApi();

    // Get transaction fee using existing API connection
    const transactionFee = await estimateUnstakeFeeWithApi(
      api,
      hotkey,
      amount,
      subnet
    ...
    if (subnet === 0) {
      ...
      try {
        ...
        const actualTaoBeforeFee =
          idealTaoValue -
          idealTaoValue * (slippageInfo.slippagePercentage / 100); // TAO after slippage, before fee
        const slippageCost = (
          idealTaoValue *
          (slippageInfo.slippagePercentage / 100)
        ).toString();

        const totalCostValue =
          parseFloat(amount) +
          parseFloat(transactionFee) +
          parseFloat(slippageCost);
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : 'Unknown error calculating slippage';
        throw new Error(
          `Cannot calculate slippage for subnet unstaking - unable to provide accurate cost estimate: ${errorMessage}`
        );
      }
    }
  }
